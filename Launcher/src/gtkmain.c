...
static void initialize_window(GtkWidget *window) {
	gtk_window_set_title(GTK_WINDOW(window), "Conquer - Launcher - 1.0.0");
...
static void insertNewElement(GtkWidget *widget, gpointer data) {
	...
	if (gtk_entry_buffer_get_length(buffer)) {
		GtkWidget *item = gtk_label_new(gtk_entry_buffer_get_text(buffer));
...
static void startConquer(ProgramData pd) {
	...
	assert(jvmoptions);
	// Just free the first optionstring and the last few.
...
int main(int argc, char **argv) {
	...
	if (c) {
		for (size_t i = 0; i < c->numOptions; i++) {
			...
			gtk_list_box_insert(GTK_LIST_BOX(jvmOptionsList), item, -1);
		}
		for (size_t i = 0; i < c->numClasspaths; i++) {
			GtkWidget *item = gtk_label_new(c->classpaths[i]);
			gtk_list_box_insert(GTK_LIST_BOX(classpathsList), item, -1);
		}
	}
	gtk_widget_show_all(window);
	gtk_main();
...
static void dumpConfiguration(Configuration c) {
	...
	cJSON *classpaths = cJSON_AddArrayToObject(main, "classpath");
	for (size_t i = 0; i < c->numClasspaths; i++) {
		cJSON_AddItemToArray(classpaths, cJSON_CreateString(c->classpaths[i]));
	}
	cJSON *options = cJSON_AddArrayToObject(main, "options");
	for (size_t i = 0; i < c->numOptions; i++) {
		cJSON_AddItemToArray(options, cJSON_CreateString(c->options[i]));
	}
	FILE *fp = fopen(jsonFile, "w");
	assert(fp);
	fputs(cJSON_Print(main), fp);
	cJSON_Delete(main);
	fclose(fp);
	free(jsonFile);
	free(base);
