...
public interface ConquerInfo extends StrategyObject, PluginInterface {
	...
	/**
	 * Bind a context to this instance.
	 * This method may be called as often as you want, until {@link #init()} was called.
	 * After calling {@link #init()}, the behavior is undefined
	...
	/**
	 * Calculate the result of the game. If more than one clan is left (Check by calling {@link #onlyOneClanAlive()} the behavior is undefined.
	 * @return The result, if only one clan is left. {@link Result#CPU_WON}, if the clan of the player is extincted, otherwise {@link Result#PLAYER_WON}.
	 */
	...
	/**
	 * Executes all strategies of the CPU. Furthermore, the current round counter is incremented, resources are produced and used, ...
	 */
	void executeActions();
	...
	/**
	 * Get a clan, based on the id. This id is obtained by calling {@link IClan#getId()}.
	 ...
	 * @return An immutable list of clannames. It should be sorted by the ascending id.
	 */
	 ...
	 * @return An immutable list of coins of all clans. It should be sorted by the ascending id.
	 */
	List<Double> getCoins();
	/**
	 * A list of the amount of colors of all clans. This list should be immutable.
	 * @return An immutable list of colors of all clans. It should be sorted by the ascending id.
	 */
	 ...
	 * @param name The name to use for saving this game state. May not be {@code null} or empty, otherwise an {@link IllegalArgumentException} shall be thrown.
	 ...
	 * @param clan The clan. May not be {@code null}, otherwise an {@link IllegalArgumentException} shall be thrown.
	 * @return {@code true} if the clan is dead, {@code false} otherwise.
	 */
	boolean isDead(final IClan clan);

	/**
	 * Returns whether the human player can play. This method can be quite redundant in a single thread, but may be useful in multithreaded scenarios.
	 ...
	 * @param clan The clan. May not be {@code null}, otherwise an {@link IllegalArgumentException} shall be thrown.
	 * @param limit The absolute limit,may not be negative, otherwise an {@link IllegalArgumentException} shall be thrown.
	 * @return
	 */
	...
	/**
	 * Sets an error handler that is called for every uncaught exception. May be ignored.
	...
	/**
	 * Set a callback function that is called as soon as the player is gifted by another clan.
	...
	/**
	 * Upgrade the defense of the given city, until the maximum level is reached or not enough coins are available.
	 * @param city The city to upgrade. May not be {@code null}.
	 */
	void upgradeDefenseFully(final ICity city);

	/**
	 * Upgrade the resource production of the given city, until the maximum level is reached or not enough coins are available.
	...
	/**
	 * Return the usage and production of the resources. The default implementation is here as a replacement for e.g. {@link Shared#STONE_PER_PERSON_PER_ROUND}.
	...
	/**
	 * Return an object describing the costs for soldiers. The default implementation is here as a replacement for e.g. {@link Shared#COINS_PER_MOVE_OF_SOLDIER}.
	...
	/**
	 * Gives the maximum level for resource production, soldier levels and so on. The default implementation is here as a replacement for {@link Shared#MAX_LEVEL}.
