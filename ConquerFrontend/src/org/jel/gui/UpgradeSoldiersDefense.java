...
final class UpgradeSoldiersDefense extends JPanel implements ActionListener {
	...
	private String getOneLevelString() {
		return Messages.getMessage("Shared.upgradeToLevel", (this.clan.getSoldiersDefenseLevel() + 1),
				Utils.format(clan.upgradeCostsForOffenseAndDefense(this.clan.getSoldiersDefenseLevel() + 1)));
	}
	...
	private void initUpgradeMax() {
		...
		final var count =clan.maxLevelsAddOffenseDefenseUpgrade(this.clan.getSoldiersDefenseLevel() + 1,
				this.clan.getCoins());
	...
	private void initUpgradeOnce() {
		...
		final var costs = clan.upgradeCostsForOffenseAndDefense(this.clan.getSoldiersDefenseLevel() + 1);
